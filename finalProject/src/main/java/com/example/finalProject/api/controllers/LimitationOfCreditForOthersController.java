...
public class LimitationOfCreditForOthersController {

	@Autowired
	private ILimitationOfCreditService limitationOfCreditService;

	@PostMapping("/limitationOfCreditForOthers/{id}")
	public ResponseEntity<LimitationOfCredit> add(@PathVariable(value="id") int customerId) {

		Customer customer = new Customer();
		customer.setId(customerId);

		LimitationOfCredit addedLimitationOfCreditService = limitationOfCreditService.addOther(customer);
